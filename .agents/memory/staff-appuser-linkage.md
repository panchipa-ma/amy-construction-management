...
External 職人 app accounts (`app_users`) are linked to `staff` records by **matching email** — either `staff.email` (連絡用) OR `staff.app_login_email` (アプリ連携用, for when the contact email is a company domain that differs from the login email), lowercased. Internally-created 工程表/出面 auto-reflect to the linked 職人's app. `employees.email` is also a linkage key. Any new email-based linkage must check BOTH staff columns. `GET /staff` also returns an internal-only computed `appLinked` boolean (matched vs approved app_users).
...
**PII guard:** `GET /staff` strips `email`, `appLoginEmail`, and `appLinked` (the linkage fields) from the response for external users; internal sees them.
