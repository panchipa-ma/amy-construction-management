...
- **見積 → 売上 自動同期**: Whenever a quote is created/updated/deleted (POST/PATCH/DELETE `/api/quotes/:id`), the linked project's `contractAmount` (= 売上 on 施工台帳) is auto-synced to the latest quote's tax-excluded subtotal via `syncProjectContractAmount(projectId)`. Deleting the only quote resets it to 0. Manual edits to 売上 on the ledger will be overridden the next time a quote is saved.
