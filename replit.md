...
- **工程表** (`/gantt`): 全案件を accordion で。`ProjectGantt` を再利用。Status (default 施工中・契約済) + 検索 (案件名・顧客名)。Phase dialog に 担当職人 dropdown — `project_phases.staffId` をセット。`ProjectGantt` の CardHeader に「PDF出力」ボタン (案件単位、顧客提出用)。**PDF format** は伝統的な日本式 工程表 シート (`components/print-gantt-sheet.tsx` の `PrintGanttSheet`) — A4 横、月ごとに 1 ページ。Header: 工事名 / 構造 / 号室 / 計画 (project.startDate) / 実施 (project.endDate) / 作成者 (siteSupervisor)。グリッド: 令和年月 + 日番号行 + 曜日行 (日=赤/土=青、週末セル灰色) + 工事項目行 (最低 12 行)。各 phase は赤線+矢印で span を表示 (月境界で clip)。複数月案件は `getMonthsForPhases(phases)` で範囲を計算し、各月のシートを画面外 `position: fixed; left: -100000` に隠して render → html2canvas-pro で順番にキャプチャ → jsPDF で各月を別ページ追加。最大 24 ヶ月まで。`useGetProject` で project info 取得。シート幅 = `LABEL_W(140) + daysInMonth * DAY_W(26)`。
